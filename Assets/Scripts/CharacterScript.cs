...
public class CharacterScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isMoving)
        {
            ...
            {
                currentSpeed *= boostFactor; // Увеличиваем скорость
            }

            Vector2 moveValue = moveAction.ReadValue<Vector2>();
            Vector3 move = Camera.main.transform.forward;  // направление взгляда камеры
            move.y = 0.0f;   // проецируем на горизонтальную плоскость

            if (move == Vector3.zero)  // если вектор был вертикальным (взгляд вниз)
            {
                move = Camera.main.transform.up;   // тогда вперед "смотрит" ось Y
            }
            move.Normalize();   // нормализуем вектор после проецирования

            Vector3 moveForward = move;  // сохраняем для поворота персонажа

            // добавляем к нему управление, которое тоже ориентировано по камере
            move += moveValue.x * Camera.main.transform.right;
            move.y = moveValue.y;
            move.y -= 30f * Time.deltaTime;   // падение
            characterController.Move(currentSpeed * Time.deltaTime * move);

            // Поворачиваем персонажа в направлении движения
            this.transform.forward = moveForward;

            // Переключаем анимации в зависимости от высоты над землей
            if (this.transform.position.y - Terrain.activeTerrain.SampleHeight(this.transform.position) > 1.5f)
            {
                animator.SetInteger("MoveState", 2); // состояние полета
            }
            else
            {
                animator.SetInteger("MoveState", 1); // состояние движения
            }
        }
        else
        {
            animator.SetInteger("MoveState", 0); // состояние остановки
        }
    }
